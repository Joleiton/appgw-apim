...
#1######
#apim/service
$_apimServiceName = "apim1internal"  # API Management service instance name   
...
#2######
#apim/service
$_apimServiceName2 = "apim2internal"  # API Management service instance name   
$_apimOrganization2 = "Microsoft"
$_apimAdminEmail2 = "joleiton@microsoft.com"

#apim/hostname
$_apim_gatewayHostname2 =  "api.apissec.com"
$_apim_portalHostname2 = "portal.apissec.com"

#Self Signed Cert Password
$_gatewayCertPfxPassword2 = "certificatePassword123"
$_PortalCertPfxPassword2 = "certificatePassword123"
...
#inbound
$inrule1 = New-AzNetworkSecurityRuleConfig -Name 'Any' -Description "Any communication" -Access Allow -Protocol Tcp -Direction Inbound -Priority 105 -SourceAddressPrefix * -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange *
$inrule104 = New-AzNetworkSecurityRuleConfig -Name 'CASG-Rule-104' -Description "CSS Governance Security Rule.  Deny risky inbound.  https://aka.ms/casg" -Access Deny -Protocol Tcp -Direction Inbound -Priority 4096 -SourceAddressPrefix Internet -SourcePortRange * -DestinationAddressPrefix Any -DestinationPortRange 13,17,19,22,23,53,69,111,119,123,135,137,138,139,161,162,389,445,512,514,593,636,873,1433,1434,1900,2049,2301,2381,3268,3306,3389,4333,5353,5432,5800,5900,5985,5986,6379,7000,7001,7199,9042,9160,9300,11211,16379,26379,27017
##
...
#Step 2 - Assing the address range 10.0.0.0/24 to the subnet variable to be used for API Management while creating a Virtual Network
$apimsubnet2 = New-AzVirtualNetworkSubnetConfig -Name "subnet-apim-2" -AddressPrefix "10.0.2.0/24"  -NetworkSecurityGroup $nsg_apim
...
$vnet = New-AzVirtualNetwork -Name "appgw-apim-vnet" -ResourceGroupName $resourceGroupName -Location $location -AddressPrefix "10.0.0.0/16" -Subnet $appgatewaysubnet,$apimsubnet, $apimsubnet2
...
$apim2subnetdata = $vnet.Subnets[2]
...
######################################################################################################
######################################################################################################
#3.2 Create an API Management service inside a VNET configured in internal mode 

#Step 1 - Create an API Management Virtual Network object using the subnet $apimsubnetdata created above
$apimVirtualNetwork = New-AzApiManagementVirtualNetwork -SubnetResourceId $apim2subnetdata.Id

#Step 2 - Create an API Management service inside the Virtual Network 

$apimServiceName = $_apimServiceName2 # API Management service instance name 

$apimOrganization = $_apimOrganization2

$apimAdminEmail = $_apimAdminEmail2

$apimService2 = New-AzApiManagement -ResourceGroupName $resourceGroupName -Location $location -Name $apimServiceName -Organization $apimOrganization -AdminEmail $apimAdminEmail -VirtualNetwork $apimVirtualNetwork -VpnType "Internal" -Sku "Developer"
...
$gatewayHostname2 = $_apim_gatewayHostname2
$portalHostname2 = $_apim_portalHostname2

#### Self Signed Certificates

$gatewayCertPfxPassword2 = $_gatewayCertPfxPassword2
$PortalCertPfxPassword2 = $_PortalCertPfxPassword2

$certGateway2 = New-SelfSignedCertificate -DnsName $gatewayHostname2 -CertStoreLocation "cert:\LocalMachine\My" -KeyLength 2048 -KeySpec "KeyExchange"

$password2 = ConvertTo-SecureString -String $gatewayCertPfxPassword2 -Force -AsPlainText

$certGatewayName2 = "gateway2"+".pfx"

Export-PfxCertificate -Cert $certGateway2 -FilePath $certGatewayName2  -Password $password2

$certPortal2 = New-SelfSignedCertificate -DnsName $portalHostname2 -CertStoreLocation "cert:\LocalMachine\My" -KeyLength 2048 -KeySpec "KeyExchange"

$password2 = ConvertTo-SecureString -String $PortalCertPfxPassword2 -Force -AsPlainText

$certPortalName2 =  "portal2"+".pfx"

Export-PfxCertificate -Cert $certPortal2 -FilePath $certPortalName2 -Password $password2

Export-Certificate -Cert $certGateway2 -FilePath gateway2.cer
####

$gatewayCertCerPath2 = "gateway2.cer"
$gatewayCertPfxPath2 = "gateway2.pfx"
$portalCertPfxPath2 = "portal2.pfx"
$gatewayCertPfxPassword2 = $_gatewayCertPfxPassword2
$PortalCertPfxPassword2 = $_PortalCertPfxPassword2

$certPwd2 = ConvertTo-SecureString -String $gatewayCertPfxPassword2 -AsPlainText -Force
$certPortalPwd2 = ConvertTo-SecureString -String $portalCertPfxPassword2 -AsPlainText -Force


#STEP 2 - Create and set the hostname configuration object for the proxy and for the portal


$proxyHostnameConfig2 =  New-AzApiManagementCustomHostnameConfiguration -Hostname $gatewayHostname2 -HostnameType Proxy -PfxPath $gatewayCertPfxPath2 -PfxPassword $certPwd2

$portalHostnameConfig2 = New-AzApiManagementCustomHostnameConfiguration -Hostname $portalHostname2 -HostnameType DeveloperPortal -PfxPath $portalCertPfxPath2 -PfxPassword $certPortalPwd2 

$apimService2.ProxyCustomHostnameConfiguration = $proxyHostnameConfig2
$apimService2.PortalCustomHostnameConfiguration = $portalHostnameConfig2

Set-AzApiManagement -InputObject $apimService2
...
$cert2 = New-AzApplicationGatewaySslCertificate -Name "certProxy02" -CertificateFile $gatewayCertPfxPath2 -Password $certPwd2

$certPortal2 = New-AzApplicationGatewaySslCertificate -Name "certPortal02" -CertificateFile $portalCertPfxPath2 -Password $certPortalPwd2
...
$listener2  =  New-AzApplicationGatewayHttpListener -Name "listener201" -Protocol "Https" -FrontendIPConfiguration $fipconfig01 -FrontendPort $fp01 -SslCertificate $cert2 -HostName $gatewayHostname2 -RequireServerNameIndication true 

$portalListener2 = New-AzApplicationGatewayHttpListener -Name "listener202" -Protocol "Https" -FrontendIPConfiguration $fipconfig01 -FrontendPort $fp01 -SslCertificate $certPortal2 -HostName $portalHostname2 -RequireServerNameIndication true
...
$apimprobe2 = New-AzApplicationGatewayProbeConfig -Name "apim2proxyprobe" -Protocol "Https" -HostName $gatewayHostname2 -Path "/status-0123456789abcdef" -Interval 30 -Timeout 120 -UnhealthyThreshold 8
$apimPortalProbe2 = New-AzApplicationGatewayProbeConfig -Name "apim2portalprobe" -Protocol "Https" -HostName $portalHostname2 -Path "/signin" -Interval 60 -Timeout 300 -UnhealthyThreshold 8
...
$authcert2 = New-AzApplicationGatewayAuthenticationCertificate -Name "whitelistcert2" -CertificateFile $gatewayCertCerPath2
...
$apimPoolSetting2 = New-AzApplicationGatewayBackendHttpSettings -Name "apimPoolSetting2" -Port 443 -Protocol "Https" -CookieBasedAffinity "Disabled" -Probe $apimprobe2 -AuthenticationCertificates $authcert2 -RequestTimeout 180

$apimPoolPortalSetting2 = New-AzApplicationGatewayBackendHttpSettings -Name "apimPoolPortalSetting2" -Port 443 -Protocol "Https" -CookieBasedAffinity "Disabled" -Probe $apimPortalProbe2 -AuthenticationCertificates $authcert2 -RequestTimeout 180
...
$apimProxyBackendPool2 = New-AzApplicationGatewayBackendAddresspool -Name "apimbackend2" -BackendIPAddresses $apimService2.PrivateIPAddresses[0] 
...
$rule201 = New-AzApplicationGatewayRequestRoutingRule -Name "rule201" -RuleType Basic -HttpListener $listener2 -BackendAddressPool $apimProxyBackendPool2 -BackendHttpSettings $apimPoolSetting2

$rule202 = New-AzApplicationGatewayRequestRoutingRule -Name "rule202" -RuleType Basic -HttpListener $portalListener2 -BackendAddressPool $apimProxyBackendPool2 -BackendHttpSettings $apimPoolPortalSetting2
...
$appgw = New-AzApplicationGateway -Name $appgwName -ResourceGroupName $resourceGroupName -Location $location -BackendAddressPools $apimProxyBackendPool,$apimProxyBackendPool2 -BackendHttpSettingsCollection $apimPoolSetting, $apimPoolPortalSetting, $apimPoolSetting2, $apimPoolPortalSetting2 -FrontendIPConfigurations $fipconfig01  -GatewayIPConfigurations $gipcongif -FrontendPorts $fp01 -HttpListeners $listener,$portalListener,$listener2,$portalListener2 -RequestRoutingRules $rule01,$rule02,$rule201,$rule202 -Sku $sku -WebApplicationFirewallConfiguration $config -SslCertificates $cert, $certPortal,$cert2,$certPortal2 -AuthenticationCertificates $authcert,$authcert2 -Probes $apimprobe,$apimPortalProbe,$apimprobe2,$apimPortalProbe2 
